...
LEFT JOIN log l ON l.userAgent = ac.codAcesso AND l.route LIKE '%getusermore%'
WHERE nl.status = 1
  AND l.id IS NULL
  AND a.codAssociado NOT IN (
    SELECT r2.codConsultRelaciona
    FROM relaciona r2
    ...
    WHERE l2.route LIKE '%getusermore%'
  )
GROUP BY r.codConsultRelaciona;