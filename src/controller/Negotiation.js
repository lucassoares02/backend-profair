        ...
        const queryReusult = await new Promise(async (resolve, reject) => {
          await Promise.all(
            results[1].map(async (negotiation) => {
              const queryConsult = `SET sql_mode = ''; select mercadoria.codMercadoria as codMercadoria, mercadoria.nomeMercadoria as nomeMercadoria,mercadoria.complemento, mercadoria.marca, mercadoria.precoUnit, mercadoria.embMercadoria, mercadoria.fatorMerc, mercadoria.highlight, mercadoria.tag, mercadoria.precoMercadoria as precoMercadoria, IFNULL(SUM(pedido.quantMercPedido), 0) as quantMercadoria FROM mercadoria left outer JOIN pedido ON(mercadoria.codMercadoria = pedido.codMercPedido) and pedido.codAssocPedido =  ${codclient}  and pedido.codNegoPedido = ${negotiation["codNegociacao"]} where mercadoria.nego = ${negotiation["codNegociacao"]} and mercadoria.codFornMerc = ${codforn} GROUP BY mercadoria.codMercadoria ORDER BY IFNULL(mercadoria.highlight, 0) DESC, nomeMercadoria, marca`;
