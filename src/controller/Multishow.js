...
const Notice = {
  async getQueryPreEvent(req, res) {
    ...
    const queryConsult =
      "select CONCAT('UPDATE multishow_b2b.negociacoes_lojas SET id_loja = 322 WHERE id_negociacao = ', codNegociacao, ';') as 'query' from negociacao";
  ...
  async getQueryPreInside(req, res) {
    ...
    const queryConsult =
      "select CONCAT('UPDATE multishow_b2b.negociacoes_lojas SET id_loja = 322 WHERE id_negociacao = ', id_negociacao, ';') as 'query' from negociacao_loja where status = 2 group by id_negociacao;";
  ...
  async getQueryPreOutside(req, res) {
    ...
    const queryConsult =
      "select CONCAT('UPDATE multishow_b2b.negociacoes_lojas SET id_loja = 322 WHERE id_negociacao = ', id_negociacao, ';') as 'query' from negociacao_loja where status = 1 group by id_negociacao;";
  ...
  async getQueryPosEvent(req, res) {
    ...
    // // QUERY QUE LIMITA SOMENTE QUEM FOI NO EVENTO
    // const queryConsult =
    //   "SET sql_mode = ''; select CONCAT(  'UPDATE multishow_b2b.negociacoes_lojas SET id_loja = ', nl.id_loja, ' WHERE id_negociacao = ', nl.id_negociacao, ' and id_negociacao_loja = ', nl.id_negociacao_loja, ';' ) as 'query' from acesso a join consultor c on c.codConsult = a.codUsuario join relaciona r on r.codAssocRelaciona = c.codConsult join associado aa on aa.codAssociado = r.codConsultRelaciona join negociacao_loja nl on nl.id_loja = aa.idLoja where a.is_present = 1 and a.direcAcesso = 2 and nl.status = 0 group by nl.id_loja, nl.id_negociacao;";

    // QUERY QUE LIMITA SOMENTE QUEM NÃO FOI NO EVENTO
    const queryConsult =
      "SET sql_mode = ''; select CONCAT(  'UPDATE multishow_b2b.negociacoes_lojas SET id_loja = ', nl.id_loja, ' WHERE id_negociacao = ', nl.id_negociacao, ' and id_negociacao_loja = ', nl.id_negociacao_loja, ';' ) as 'query' from acesso a join consultor c on c.codConsult = a.codUsuario join relaciona r on r.codAssocRelaciona = c.codConsult join associado aa on aa.codAssociado = r.codConsultRelaciona join negociacao_loja nl on nl.id_loja = aa.idLoja where a.is_present = 0 and a.direcAcesso = 2 and nl.status = 0 group by nl.id_loja, nl.id_negociacao;";

    // QUERY PARA TODOS
    // const queryConsult =
    //   "SET sql_mode = ''; select CONCAT('UPDATE multishow_b2b.negociacoes_lojas SET id_loja = ', nl.id_loja, ' WHERE id_negociacao = ', nl.id_negociacao, ' and id_negociacao_loja = ', nl.id_negociacao_loja, ';' ) as 'query' from associado aa join negociacao_loja nl on nl.id_loja = aa.idLoja where nl.status = 0 group by nl.id_loja, nl.id_negociacao;";
  ...
  async insertDataAll(req, res) {
    ...
    try {
      ...
      const groups = await Notice.getGroups();
      await Notice.insertGroups(groups);
  ...
  getGroups() {
    ...
    const query = `select * from multishow_b2b.grupos;`;

    return new Promise((resolve, reject) => {
      connectionMultishow.query(query, (error, groups, fields) => {
        if (error) {
          logger.error(`Error Connection Multishow Groups: ${error}`);
          reject(error);
        } else {
          resolve(groups);
        }
